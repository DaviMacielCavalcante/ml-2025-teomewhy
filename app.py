...
redes_options = ["Linkedin", "Twich", "Youtube", "Instagram", "Amigos", "Twitter / X", "Outra rede social"]
...
with col1:
    ...
    tempos_options = ["De 0 a 6 meses", "De 6 meses a 1 ano","De 1 a 2 anos", "de 2 anos a 4 anos", "Mais de 4 anos", "Não atuo"]

    tempos = st.selectbox("Tempo que atua na área de dados", options=tempos_options)
    
with col2:
    livros = st.radio("Curte livros?", ["Sim", "Não"])
    tabuleiro = st.radio("Curte jogos de tabuleiro?", ["Sim", "Não"])
    
    areas_options = ["Biológicas", "Exatas", "Humanas"]
    
    formacao = st.selectbox("Área de Formação", options=areas_options)
    
    niveis_options = [
        "Iniciante", "Júnior", "Pleno", "Sênior",
        "Especialista", "Coordenação", "Gerência",
        "Diretoria", "C-Level"
    ]
    
    cadeira = st.selectbox("Posição da cadeira (senioridade)", options=niveis_options)
...
with col3:
    ...
    uf = st.selectbox("Estado que mora atualmente", options=estados_options)
...
data = {
    ...
    'Tempo que atua na área de dados': tempos, 
...
dummy_vars = [
    ...
    'Estado que mora atualmente','Curte games?',
    'Área de Formação',
...
df_template = pd.DataFrame(columns=['Curte games?', 'Como conheceu o Téo Me Why?_Amigos',
...
'Posição da cadeira (senioridade)_Sênior', 'Curte futebol?',
...
if proba > 0.7:
    st.success(f"Acho, com {proba:.2f} de certeza, que você é feliz!")
elif proba > 0.4:
    st.warning(f"Acho, com {proba:.2f} de certeza, que você é feliz!")
elif proba < 0.4:
    st.error(f"Acho, com {proba:.2f} de certeza, que você esta triste!")
